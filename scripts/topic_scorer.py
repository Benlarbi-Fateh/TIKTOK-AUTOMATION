...
PROMPT_PATH = PROJECT_ROOT / "prompts" / "score_topic.txt"
...
def load_prompt() -> str:
    """
    Charge le modèle de prompt utilisé pour noter les sujets.
    """
...
def build_prompt(topic: dict[str, Any]) -> str:
    """
    Remplace les variables du prompt par les données du sujet.
    """
...
def validate_scores(
    ...
    """
    Vérifie les champs et les limites des scores produits par Ollama.
    """
...
def score_topic(topic_id: int) -> dict[str, Any]:
    """
    Lit un sujet dans SQLite, le fait analyser par Ollama,
    puis enregistre ses scores dans la base de données.
    """
